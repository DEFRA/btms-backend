...
public static class CommoditiesMapper
{
	public static Btms.Model.Ipaffs.Commodities? Map(Btms.Types.Ipaffs.Commodities? from)
	{
        ...
        to.CommodityComplements = from.CommodityComplements?.Select(x => CommodityComplementMapper.Map(x)).Where(x => x != null).ToArray()!;
        to.ComplementParameterSets = from.ComplementParameterSets?.Select(x => ComplementParameterSetMapper.Map(x)).Where(x => x != null).ToArray()!;
