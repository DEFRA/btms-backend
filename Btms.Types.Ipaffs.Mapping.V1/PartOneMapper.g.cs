...
{
	public static Btms.Model.Ipaffs.PartOne? Map(Btms.Types.Ipaffs.PartOne? from)
	{
		if (from == default) return default;

		var to = new Btms.Model.Ipaffs.PartOne ();
        ...
        to.AdditionalPermanentAddresses = from.AdditionalPermanentAddresses?.Select(x => EconomicOperatorMapper.Map(x)).Where(x => x != null).ToArray()!;
        ...
        to.SealsContainers = from.SealsContainers?.Select(x => SealContainerMapper.Map(x)).Where(x => x != null).ToArray()!;
        ...
        to.NominatedContacts = from.NominatedContacts?.Select(x => NominatedContactMapper.Map(x)).Where(x => x != null).ToArray()!;
