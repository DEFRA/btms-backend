...
////public record DecisionResult(List<DecisionClearanceRequest> ClearanceRequests);

public record DecisionResult
{
    private readonly List<MovementDecisionResult> _results = [];
    public MovementDecisionResult AddDecision(string entryReference, int entryVersion)
    {
        var item = new MovementDecisionResult(entryReference, entryVersion);
        _results.Add(item);
        return item;
    }

    public IReadOnlyList<MovementDecisionResult> MovementDecisions => _results.AsReadOnly();
}
...
public record DocumentDecisionResult(Document Document, DecisionCode DecisionCode)
{
    ////public string Reason =>
    ////    $"A Customs Declaration has been submitted however no matching {MapToChedType(Document.DocumentCode!)}(s) have been submitted to Port Health (for {MapToChedType(Document.DocumentCode!))} number(s) {Document.DocumentReference}). Please correct the {MapToChedType(Document.DocumentCode!)} number(s) entered on your customs declaration.";
